...
export function VisualSlot({ visual }: { visual: Visual }) {
  if (visual.src) {
    return (
      <figure className="overflow-hidden rounded-[14px] bg-card-soft">
        <div>
          {visual.kind === 'video' ? (
  ...
  return (
    <figure className="flex min-h-40 flex-col justify-end rounded-[14px] border border-dashed border-border bg-card-soft p-4">
