    ...
    <Reveal as="section" className="border-t border-border py-12 md:py-16">
      ...
      <div id={chapter.id} className="scroll-mt-28">
        ...
        <div className="min-w-0">
          ...
          {chapter.decision && (
            <aside className="mt-6 max-w-2xl rounded-[14px] border-l-2 border-track-strategy bg-card-soft p-4 md:p-5">
          ...
          {chapter.metrics && chapter.metrics.length > 0 && (
            <dl className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-3">
              {chapter.metrics.map((m) => (
                ...
                <div key={m.label} className="flex flex-col-reverse rounded-[14px] bg-card-soft p-4">
      ...
      <main id="main" className="mx-auto max-w-6xl px-6 pb-20 pt-32 md:px-10 lg:px-14">
          ...
          <ol className="flex gap-3 overflow-x-auto pb-2">
            {page.chapters.map((c, i) => (
              <li key={c.id} className="min-w-[160px] flex-1">
                <a href={`#${c.id}`} className="block rounded-[14px] bg-card-soft transition-transform hover:-translate-y-0.5">
                  <div className="p-3">
        ...
        <Reveal className="mt-8">
          <dl className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {page.metrics.map((m) => (
              <div key={m.label} className="flex flex-col-reverse rounded-[14px] bg-card-soft p-5">
        ...
        {next && (
          <Reveal as="section" className="border-t border-border pt-10">
            <Link
              ...
              className="group block rounded-[14px] bg-card-soft p-6 transition-transform hover:-translate-y-0.5 md:p-8"
            >
        ...
        <Reveal as="section" className="border-t border-border pt-10">
          ...
          <ul className="mt-4 grid gap-4 md:grid-cols-3">
            {others.map((p) => (
              <li key={p.slug}>
                <Link href={p.href} className="block h-full rounded-[14px] bg-card-soft transition-transform hover:-translate-y-0.5">
